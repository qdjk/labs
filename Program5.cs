...
namespace lab1
{
    ...
    public class Group : IIdentifiable
    {
        ...
        public double GetAverageGroupScore()
        {
    ...
    public class Student : IIdentifiable, IGradeable, IReportable
    {
        ...
        public void DisplayInfo()
        {
            ...
            Console.WriteLine($"Средний балл: {Score:F2}");
        }
        ...
        public string GenerateReport()
        {
            ...
            sb.AppendLine($"Краткий отчет по студенту");
            sb.AppendLine($"ФИО: {Name}");
            sb.AppendLine($"Группа: {Group?.Name ?? "Не указана"}");
            sb.AppendLine($"Факультет: {Faculty?.Name ?? "Не указан"}");
            sb.AppendLine($"Университет: {University?.Name ?? "Не указан"}");
            sb.AppendLine($"Средний балл: {Score}");
            sb.AppendLine($"Статус: {(IsExcellent() ? "Отличник" : "Не отличник")}");
            sb.AppendLine($".....................................................................");
            return sb.ToString();
        }
    }
    // Базовый процессор событий студентов
    public class StudentProcessorBase
    {
        // Само событие (подписчики добавляются сюда)
        public event StudentOperation StudentOperation;

        // Виртуальная "точка входа" для вызова обработчиков — можно переопределить
        protected virtual void OnStudentOperation(Student student)
        {
            StudentOperation?.Invoke(student);
        }

        // Публичный метод для вызова обработчиков
        public void Invoke(Student student)
        {
            OnStudentOperation(student);
        }
    }

    // Наследник, который переопределяет поведение вызова события и добавляет обработку исключений
    public class SafeStudentProcessor : StudentProcessorBase
    {
        protected override void OnStudentOperation(Student student)
        {
            try
            {
                // Вызвать всех подписчиков (базовая реализация)
                base.OnStudentOperation(student);
            }
            catch (StackOverflowException ex)
            {
                // Примечание: в большинстве версий .NET StackOverflowException приводит к завершению процесса,
                // его поймать и продолжить работу часто невозможно. Тем не менее, для учебных целей
                // оставлен catch-блок.
                Console.WriteLine($"Обнаружен StackOverflowException при обработке студента {student.Name}: {ex.Message}");
            }
            ...
            {
                // OutOfMemoryException может возникнуть при нехватке памяти — осторожно продолжать выполнение.
            ...
            {
                // Общий fallback — если какое-то другое исключение просочилось
                Console.WriteLine($"Необработанное исключение ({ex.GetType().Name}): {ex.Message}");
            }
        }
    }
    ...
    {
        // Статический процессор (используем безопасный)
        static StudentProcessorBase processor = new SafeStudentProcessor();

        // Проброс события на уровень Program (требование: public event StudentOperation)
        public static event StudentOperation StudentOperation
        {
            add { processor.StudentOperation += value; }
            remove { processor.StudentOperation -= value; }
        }
        static List<University> universities = new List<University>();

        // Методы для работы с делегатом 
        static void DisplayStudentInfo(Student student)
        {
            Console.WriteLine("Собираем информацию!!!");
            student.DisplayInfo();
            Console.WriteLine();
        }
        static void CheckStudentStatus(Student student)
        {
            string status = student.IsExcellent() ? "отличником" : "не отличником";
            Console.WriteLine($"Студент {student.Name} является {status}");
            Console.WriteLine();
        }
        static void GenerateStudentReport(Student student)
        {
            string report = student.GenerateReport();
            Console.WriteLine(report);
        }
        static void ProcessStudentWithDelegate(Student student)
        {
          
            StudentOperation += DisplayStudentInfo;
            StudentOperation += CheckStudentStatus;
            StudentOperation += GenerateStudentReport;

            Console.WriteLine($"Вся информация о студенте: {student.Name} ");

            
            processor?.Invoke(student);
        ...
        static void ProcessAllStudents()
        {
            ...
            foreach (var uni in universities)
            {
                foreach (var fac in uni.Faculties)
                {
                    foreach (var group in fac.Groups)
                    {
                        ...
                        {
                            ProcessStudentWithDelegate(student);
                            processedCount++;
                        }
                    }
                }
            ...
            Console.WriteLine($"Обработано студентов: {processedCount}");
        }
        ...
        static void Case()
        {
            while (true)
            {
                ...
                Console.WriteLine("4. Сохранить топ отличников среди институтов и групп");
                Console.WriteLine("5. Вся информация о каждом студенте");
